...
def get_data_from_gz(source_path, target_path):
    ...
    if main_config_read.get("backup-data", False) is True:  # 是否备份数据，在主配置文件中配置
        ...
        log_collect_append.logger.info(target)
        shutil.copy(source_path, target)
    code = shell("gzip -d %s" % source_path)  # 调用shell命令gzip用于解压gz文件
    if code == 0:
        os.rename(name, rename)
        log_collect_append.logger.info("Rename: %s to %s" % (name, rename))
...
class DataProcess():
    ...
    def process(self):
        ...
        if BOOL is True:
            ...
            log_collect_append.logger.debug("Run Command: %s" % cmd)
            code = shell(cmd)
            if code == 0:
                path = self.backUp()
    ...
    def backUp(self):
        ...
        return_folder = "None"
        log_collect_append.logger.info(fileList)
        for i in fileList:
            ...
            if os.path.isdir(tmp):
                for j in os.listdir(tmp):
                    if "Summary_Report" in j:
                        ...
                        log_collect_append.logger.debug(
                            "Copy File: %s to %s" % (str(os.path.join(tmp, j)), str(xlsxFolder)))
