...
class Scrapy_Data():
    ...
    def parametricSubmit(self, parameterDict):
        ...
        log_collect_append.logger.debug(fullUrl)
        log_collect_append.logger.debug(headers)
        ...
        log_collect_append.logger.debug("""parametricSubmit
        Start: %s
        End: %s
        Finish: %s""" % (
    ...
    def decodeParametricStatus(self, taskStatus, states_json_path, stationList, taskID):
        ...
        log_collect_append.logger.info(dict_info)
    ...
    def parametricDownload(self, filename, taskID, stationList, states_json_path, sleepSeconds=30, maxTimeInMin=60,
        ...
        log_collect_append.logger.debug("""parametric queue finish
    	Start: %s
    	End: %s
    	Finish: %s""" % (
            start.strftime('%Y-%m-%s %H:%M:%S'), end.strftime('%Y-%m-%s %H:%M:%S'), str((end - start).total_seconds())))
        ...
        if counter > (maxTimeInMin * 60) / sleepSeconds:
            log_collect_append.logger.info('Process Took longer than {} minutes so was killed'.format(maxTimeInMin))

        return filename, taskResponse

    def request_task(self, payload):
        log_collect_append.logger.info(payload)
