...
import xlwt
import xlrd
from functions.check_history_summary import *
...
def excel_read(file_, sheet, header=True):
    ...
    try:
        ...
        nrows = table.nrows
        log_collect_append.logger.debug(data)
        log_collect_append.logger.debug(table)
        log_collect_append.logger.debug(nrows)
...
def read_summary_report(file, frequency, radar):
    ...
    log_collect_append.logger.info(list_read)
    ...
    log_collect_append.logger.info(date_list)
...
class SummaryData():
    ...
    def merge_report(self, dictInfo, product):
        ...
        file_list = set_double_list(dictInfo)  # 双层list没法去重

        log_collect_append.logger.debug(result_folder)
        log_collect_append.logger.debug(file_list)
        ...
        if len(file_list) > 0:
            for i in file_list:
                ...
                log_collect_append.logger.info(str(frequency) + "," + str(radar))
                log_collect_append.logger.info(i[1])

                read_result = read_summary_report(str(i[1]), str(frequency), str(radar))

                # os.system("rm -rf %s" % str(i[1]))  # 读取文件之后删除excel文件

                log_collect_append.logger.debug(read_result)
                ...
                log_collect_append.logger.info("------------------------")
                log_collect_append.logger.info(summary_list)
                log_collect_append.logger.info(total_list)
                log_collect_append.logger.info(date_list)
                log_collect_append.logger.info("------------------------")
