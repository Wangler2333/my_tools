...
class mytools(forLogCheck):
    ...
    def running(self, file, worktype):
        ...
        if sn != None or folderpath != None or logpath != None:
            time1, a = self.readLogTime(logpath, 'Comm.Send.TestEnvInfo')
            time2, b = self.readLogTime(logpath, 'Comm.Send.Results', 'PASS_xVT')
            ab = sn + ',' + str(time1) + ',' + str(time2)
    ...
    def writeCsv(self, string):
        if not os.path.isfile(self.result):
            ret = 'SerialNumber' + ',' + 'TestEnvInfo' + ',' + 'ResultsTime'
