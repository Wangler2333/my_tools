    ...
    def check_file(self):
        if not os.path.isfile(self.default):
            info = {
                ...
                'restore': '/Users/gdlocal/Desktop/Restore*.txt',
                'restore_key1': 'UNIT',
                'restore_key2': 'STATION_OVERLAY_VERSION',
                'restore_key3': 'LINE',
                'ip_info':
                    {
                        'F16-1FT-C16': {
                ...
                for j in a['ip_info'][i].keys():
                    for o in a['ip_info'][i][j].keys():
                        ...
                        if self.cmd_choose == 1:
                            ...
                            if t.ret == 0:
                                td = str(states).replace(self.check_key, '').replace('\\n', '').replace('\\t',
                                                                                                        '').replace(
                                    '\\r', '')
                                d = re.findall(r'\w+', td)
                                if len(d) > 0:
                            ...
                            de = t.read_disk_diskutil()
                            t.cmd = 'cat < %s' % self.restorefile
                            restoreinfo = t.pexpect_spaw()

                            if t.ret == 0:
                                if "No such file or directory" not in str(t.r):
                                    for l in str(t.r).split('\\r\\n'):
                                        if a['restore_key3'] in l:
                                            line = l.split('=')[1]
                                        if a['restore_key1'] in l:
                                            station = l.split('=')[1]
                                        if a['restore_key2'] in l:
                                            overlay = l.split('=')[1]
                                    otherinfo = line, station, overlay
                                    overlay_version = process(otherinfo)
                                else:
                                    overlay_version = "None"
                            else:
                                overlay_version = "None"

                            result = i, j, o, a['ip_info'][i][j][o], de, p, overlay_version
        ...
        if self.cmd_choose == 2:
            ...
            for i in list:
                if '%' in str(i[4][2]):
                    ...
                    rate = "T:" + str(i[4][0]) + "GB " + "F:" + str(i[4][1]) + "GB " + "使用率: " + d
                else:
                    rate = '\033[0;31m' + str(i[4]) + '\033[030m'

                if i[5] == "ON":
                    st = '\033[0;32m' + i[5] + '\033[030m'
                elif i[5] == "OFF":
                    st = '\033[0;31m' + i[5] + '\033[030m'
                else:
                    st = '\033[0;34m' + i[5] + '\033[030m'


                print(i[0] + '\t\t' + i[1] + '\t\t' + i[2] + '\t\t' + i[3] + '\t\t' + st + '\t' + i[6] + '\t' + rate)
            print('--------------------------------------------------------------------------\n')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] in ['-h', '-H', '-help', '--']:
            ...
            print('1.0.6: 扩展字符匹配的局限性.')
            print('1.0.7: 区分状态显示颜色，显示当前Overlay版本信息.')
            print('Version: 1.0.7 测试版')
            os._exit(1)
