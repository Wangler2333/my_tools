...
# Created by TB on 2017/03/04
# Version 1.3
# Base on 1.2 / Add Preburn Log Check . Only change Location = Preburn
        ...
        with open(filePath) as msg:
            for line in msg:
                if "FAIL-UPDA" in line:
                    ...
                    d = line.split('"')[-8]
                    tol = a + " #" + b + " " + c + " " + d + ','
                    if "Left" in line.split('"')[25] or "Right" in line.split('"')[25]:
                        e = line.split('"')[25].replace('&','=').split('=')[1]
                        tol = a + " #" + b + " " + c + " " + d + ',' + e
            ...
            for le in Set_2:
                if "Passed on Retry" in le:
                    ...
                    #ae = str(le).split('Passed on Retry')[0]
                    #print ae
                    Set_2.remove(le)
                    Set_2.remove(ae)
              #  if "4274" in le and "Passed on Retry" in le:
              #      ae = le.replace('Passed on Retry', '')
              #      Set_2.remove(le)
              #      Set_2.remove(ae)
...
def Check_OutFile(file):
    try:
        if not os.path.isfile(file):
            title = "Unit Number, Serial Number, Config, Location, Error Message, Fail Port, Bundle"
