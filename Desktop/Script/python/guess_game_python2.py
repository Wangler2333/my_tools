...
class guess_number(object):
    ...
    def guess(self):
        while True:
            guessNumber = raw_input('请输入你猜测的数字:')
...
if __name__ == '__main__':
    ...
    if '-r' in dict.keys():
        randomly_ = dict['-r']
        if str(randomly_) == 'True':
            randomly = True
        elif str(randomly_) == 'False':
            randomly = False

    t = guess_number(upper_limit=upper_limit, lower_limit=lower_limit)
    t.cycle_time(times=times, randomly=randomly)
