...
TargetDirHelium=$TargetDir/$Project"_Helium"
...
createSubFolder()
{
		mkdir $TargetDirHelium/$NewFolderName
}
...
OverallLog=`find $SearchDir -type f -iname *.tgz -print | wc -l | sed 's/\ //g'`
    ...
    for i in `find $SearchDir -type f -iname *.tgz -print`; do      	
		...
		#Helium
		cp -rf $TmpDir/$FolderName/_APPLEINTERNAL_DIAGNOSTICS_LOGS/Logs/Helium/* $TargetDirHelium/$NewFolderName &>/dev/null
