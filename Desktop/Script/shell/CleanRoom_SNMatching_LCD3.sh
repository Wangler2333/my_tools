...
# 10-13-2014 release 1.11_ENG Phoenix Meng.
	# Modify LCD3 same as LCD1/2 to output result file into TIA server.Only for experiment on J44A.

		
scriptVersion="1.11_ENG"
resultFlag=""
cleanRoom_SI_Report=""
stationFlag=""
...
#		cleanRoom_SI_Report=`cat /Phoenix/Logs/SerialNumber.txt`
		outputFileName=`cat /Phoenix/Logs/SerialNumber.txt`
		outputFileName="${outputFileName}""_""LCD3.txt"
	fi
	
	if [ "${resultFlag}" == "pass" ]; then
		cleanRoom_SI_Report="${programmedSN}"",""LCD3"",""${timeStamp}"",""P"","
	else	
		cleanRoom_SI_Report="${programmedSN}"",""LCD3"",""${timeStamp}"",""F"","
	fi 
	...
	# Now it's ready to ftp the file into TIA server with retry logic in place.
	# Ping check TIA server first.
	ping -c 2 10.0.0.92 >/dev/null 2>&1
	if [ `echo $?` -ne 0 ]
	then
		echo "Can not ping server 10.0.0.92."
		exit -53
	fi		 
	...
	do 
		retryNumber=`expr ${retryNumber} + 1`
		waitTime=$((2**${retryNumber}))
		ftp -du ftp://test:test@10.0.0.92/%2F"UnitID Folder"/sfs/LCD3/ "${outputFileName}"
		# make sure ftp succeeded.
		if [ `echo $?` -ne 0 ]
		then
			echo "ftp command tries:<${retryNumber}>."
...
ll_SNmatching()
{
    ...
    if [ ! `echo "${processTrackingSN}" | wc -c | sed  's/[[^ ]]*//g'` -gt 1 ]; then
    ...
    outputFileName="${processTrackingSN}""LLCUC.txt"
