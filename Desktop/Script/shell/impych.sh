...
function usage
{
	...
	printf "    -d: Pick the H9m probe that is connected the specified device {x589kblu, j137} (case-sensitive).\n"
...
while getopts ":pfhaxd:" opt; do
	case $opt in
		...
		a)
			...
			;;
		d)
			if [ "$OPTARG" = "x589kblu" ] || [ "$OPTARG" = "j137" ]; then
...
PROBES=($(astrisctl list | grep -iE 'Chimp|Kanzi'))
PROBECOUNT=${#PROBES[@]}

if [ $PROBECOUNT -ne 0 ]; then
	printf "%u probes found.\n" ${#PROBES[@]}
else
	printf "ERROR: No Chimp/Kanzi probes found.\n"
	exit -2
fi

printf "NOTE: [Boilerplate warning] A Kanzi (via a 10-pin flex) and a Chimp should not be connected to the same H9m target.\n"
...
for probe in ${PROBES[@]}; do
	...
	# Is it a Kanzi or a Chimp ?
	IS_KANZI=$(echo $probe | grep -i kanzi)
	...
	if [ "$IS_KANZI" = "" ]; then
		...
		if [ $TGTUART_RELAY -ne 0 ]; then #x86
			...
			printf "NOTE: Probe %s is an AP probe - *may* lazily check for H9m presence.\n" $probe
	...
	for probe in ${AP_PROBE[@]}; do
		...
		if [ "$(astris --script $ASTRIS_EXPLORER_FILE $probe | grep Gibraltar)" != "" ]; then
			...
			if [ $LIST_PROBES_ONLY -eq 0 ]; then
				...
				sleep 1
				astrisctl --probe $probe relay dbgfifo 1 >> /dev/null
				#loopset_relay_on_probe $probe dbgfifo 1
			fi
...
if [ "$SELECTED_DEVICE" != "" ]; then
	...
	if [ ${#x86_PROBE[@]} -gt 1 ]; then
		...
		printf "QUERY: Which of these x86 probes is the one attached to XB on your device ?\n\t>> On the J137, XB is the port closer to the left end of the fixture.\n\t>> On the X589, XB is closer to the front of the acrylic.\n\n"
	...
	if [ "$LAUNCH_SHELL_TO_x86_PROBE" != "" ]; then
		...
		if [ $? -ne 0 ]; then
			...
			osascript -e 'tell application "Terminal"
				#activate
				do script "nanokdp --timestamp=\"%T.ssss x86> \" -d '$LAUNCH_SHELL_TO_x86_PROBE' -c 1250000,n,8,1 -K -k 0"
			end tell' >> /dev/null
		else
	...
	if [ "$LAUNCH_SHELL_TO_H9M_PROBE" != "" ]; then
		...
		if [ $? -ne 0 ]; then
			...
			if [ -f "/usr/local/lib/nanocom/libnanocom_udp.dylib" ]; then
				serial_log_reader='nanocom --timestamp=\"%T.ssss arm> \" -P /usr/local/lib/nanocom/libnanocom_udp.dylib -d udp://127.0.0.1:'
			fi
			H9M_SERIAL_STREAM="${serial_log_reader}31337"
			SMC_SERIAL_STREAM="${serial_log_reader}31339"
			osascript -e 'tell application "Terminal"
				activate
