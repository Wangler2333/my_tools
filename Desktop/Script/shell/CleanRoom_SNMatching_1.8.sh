...
# 01-06-2015 release 1.8 Phoenix Meng.
	#For X304/425 CleanRoom Bundle,remove REIN&ROE as not use any more,add LL&CU;
	#LCD2 output result using LCDSN instead of CameraSN;
...
lcd1_SNmatching()
{
    ...
    if [ `echo $?` -ne 0 ]; then
        echo "Error with parsing programmed SN from processlog.plog."
        exit -34
    fi
...
lcd2_SNmatching()
{
	...
	#1.8 Using LCD SN instead of CameraSN
...
ll_SNmatching()
{
    ...
    # Verify that SerialNumber.txt file existence and get process tracking SN as well as rough checking.
    if [ ! -f /Volumes/MaxDisk/Phoenix/Logs/SerialNumber.txt ]; then
        echo "Did not see file /Volumes/MaxDisk/Phoenix/Logs/SerialNumber.txt"
        exit -61
    fi
    processTrackingSN=`cat /Volumes/MaxDisk/Phoenix/Logs/SerialNumber.txt`
    if [ ! `echo "${fixtureCameraSN}" | wc -c | sed  's/[[^ ]]*//g'` -gt 1 ]; then
        echo "Length verification for process tracking SN failed."
        exit -62
    fi

    # Verify process.plog file existence and get fixture camera SN
	if [ ! -f /Volumes/MaxDisk/Phoenix/Logs/processlog.plog ]; then
        echo "Did not see file /Volumes/MaxDisk/Phoenix/Logs/processlog.plog"
        exit -63
    fi
    ...
    # Get programmed display panel SN and do rouch check.
    panelDetectOutput=`/Indy/Runtime_Files/OS/TestApps/PanelDetect.app/Contents/MacOS/PanelDetect -c`
    programmedDisplaySN=`echo "${panelDetectOutput}" | grep -i 'Serial' | cut -d : -f 2 | sed 's/[[^ ]]*//g'`
    if [ `echo $?` -ne 0 ]; then
        echo "Errow with parsing programmed SN from processlog.plog."
    ...
    outputFileName="${processTrackingSN}""LLCU.txt"

    # Generate final txt file following the rule.
    if [ "${resultFlag}" == "pass" ]; then
		cleanRoom_SI_Report="${processTrackingSN}"",""LLCU"",""${timeStamp}"",""P"","
	else	
		cleanRoom_SI_Report="${processTrackingSN}"",""LLCU"",""${timeStamp}"",""F"","
	fi
    ...
    # Upload txt file to server. For LL, we need to upload to 10.0.0.91/UnitID Folder/sfs/LLCUC
	# Ping check TIA server first.
	ping -c 2 10.0.0.91 >/dev/null 2>&1
	if [ `echo $?` -ne 0 ]
	then
...
cu_SNmatching()
{
    ...
    # Define txt file name.
    outputFileName="${processTrackingSN}""CLUT.txt"

    # Generate final txt file following the rule.
    if [ "${resultFlag}" == "pass" ]; then
		cleanRoom_SI_Report="${processTrackingSN}"",""CLUT"",""${timeStamp}"",""P"","
	else	
		cleanRoom_SI_Report="${processTrackingSN}"",""CLUT"",""${timeStamp}"",""F"","
	fi
	cleanRoom_SI_Report="${cleanRoom_SI_Report}""${programmedDisplaySN}"
    ...
    # Upload txt file to server. For LL, we need to upload to 10.0.0.91/UnitID Folder/sfs/CLUT
	# Ping check TIA server first.
	ping -c 2 10.0.0.91 >/dev/null 2>&1
	if [ `echo $?` -ne 0 ]
	then
