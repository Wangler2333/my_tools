...
TargetDirTBTR2D=$TargetDir/$Project"_TBTR2D"
...
createSubFolder()
{
		mkdir $TargetDirTBTR2D/$NewFolderName
}
...
OverallLog=`find $SearchDir -type f -iname *.tgz -print | wc -l | sed 's/\ //g'`
    ...
    for i in `find $SearchDir -type f -iname *.tgz -print`; do      	
		...
		#TBT PCIe R2D, 4500
		cp -rf $TmpDir/$FolderName/_APPLEINTERNAL_DIAGNOSTICS_LOGS/Logs/TBTEyeLogs/* $TargetDirTBTR2D/$NewFolderName &>/dev/null
