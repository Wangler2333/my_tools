...
Resources = os.path.join(os.path.expanduser('~'), 'Documents', 'OverlayCheck')
config_path = os.path.join(os.path.dirname(sys.argv[0]).replace('MacOS', 'Resources'), 'config.json')
...
write_history(os.path.join(os.path.dirname(sys.argv[0]).replace('MacOS', 'Resources'), 'history.txt'), history)
...
class MainFunction(QMainWindow, Ui_OverlayCheck):
    ...
    def start(self):
        '''
        开始控件连接函数：
             建立多线程类实例化，给线程机种名称参数，监听线程返回信号，
             把线程返回的字典提供给run_state函数进行下一步操作
             禁掉开始按钮
        '''
    ...
    def run_state(self, dict_info):
        '''
        接收线程所返回的信息，进行相应的分类提交给不同的函数进行处理
        '''
