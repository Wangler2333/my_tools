    ...
    for i in station_list:
        ...
        for j in line_list:
            ...
            if len(final_dict[i][j]['pass list']) > 0:
                default = final_dict[i][j]['pass list'][0][8]
            elif len(final_dict[i][j]['fail list']) > 0:
                default = final_dict[i][j]['fail list'][0][8]  # 修正当无pass station状态是 default version无法读取
...
def excel_write(product, temp, line_list, station_list, final_report):
    ...
    for j in station_list:
        ...
        result.append(str(issues).replace("',", " ").replace("[", "").replace("]", "").replace("'", ""))
...
#
# file_process(read_json_file("/Users/saseny/Desktop/OverlayCheck/config.json"),
#              "/Users/saseny/Desktop/OverlayCheck/OverlayCheck/tmp",
#              "J79A")
