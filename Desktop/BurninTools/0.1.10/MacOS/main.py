...
class MainTools(QMainWindow, Ui_BurninTools):
    ...
    def setComboBox(self):
        try:
            ...
            if self.historyInfo.get("function", False) is not False:
                ...
                if self.configInfo["function"][self.historyInfo["function"]].get("default", False) is True:
                    self.start()
        except:
    ...
    def checkState(self, dict):
        ...
        if dict["action"] == "Observation Report":
            self.report_observation(dict)
        if dict["action"] == "Detail DTI Info":
            if dict.get("finished", False) is True:
                self.label_2.setText(dict["message"])
                self.label_2.setStyleSheet("background-color: rgb(0, 255, 0);")
                self.runTime.stop()
                self.setButtonFunction(True)

    def report_observation(self, dict):
        self.label_2.setText(dict["message"])
        if dict.get("finished", False) is True:
    ...
    def observation_report(self):
        self.additional = {}
        while True:
            num, ok = QInputDialog.getInt(self, "请输入天车上当前机器总数:", "总数")
            if ok:
                self.additional["total"] = num
                break

        while True:
            pathRoad = QFileDialog.getExistingDirectory(self, "选取Log存放文件夹", '/')
            if pathRoad:
                self.additional["path"] = pathRoad
                break
    ...
    def yield_report(self, info):
        ...
        if info["info"].get("exception", False) is True and self.showed is False:
            reply = QMessageBox.information(self, "提醒", "请先保留该机器,待 HWTE 确认!!!", QMessageBox.Yes | QMessageBox.No,
                                            QMessageBox.Yes)
            self.showed = True
