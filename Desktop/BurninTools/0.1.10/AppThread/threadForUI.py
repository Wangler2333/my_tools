...
from Functions.observation_report import *
from Functions.readUnitsVersion import *
...
class WorkRunning(QThread):
    ...
    def __init__(self, configInfo, action, unitsInfo, errorInfo, errorMessage, additional=None, parent=None):
    ...
    def run(self):
        ...
        if self.action == "Observation Report":
            tmpDict = {
                "action": self.action,
                "message": "开始整理报表,请等待...",
                "finished": False
            ...
            time.sleep(0.5)
            self.runSignal.emit(tmpDict)
            t = ObservationReport(self.configInfo, self.unitsInfo, self.additional)
            t.files_list("/tmp/temp.json")
            tmpDict["finished"] = True
            time.sleep(0.5)
            self.runSignal.emit(tmpDict)

        if self.action == "Detail DTI Info":
            t = DTIDetailVersion(read_json_file(configJsonPath))
            run_info = t.running()
            tmpDict = {
                "action": self.action,
                "message": "处理结束!",
