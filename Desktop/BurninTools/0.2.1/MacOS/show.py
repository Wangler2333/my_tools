...
class ShowError(QMainWindow, Ui_show):
    ...
    def readRuninTime(self):
        timeInfo = None
        try:
            path_road = self.info.get("folder", False)
            if path_road is not False:
                path = os.path.join(path_road, "summary.csv")
                if os.path.isfile(path):
                    info = file_read(path)
                    if "Earthbound/Overall Cycle Time" in info[-1]:
                        timeInfo = str(info[-1]).split(",")[-3]
                        tmp = str(timeInfo).split(":")
                        hour = str(tmp[0]) + "时"
                        minute = str(tmp[1]) + "分"
                        second = str(tmp[2]) + "秒"
                        result = " ".join([hour, minute, second])
                        self.runTime.setText(str(result))
        except:
            pass
